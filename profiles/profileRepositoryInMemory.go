...
func (r *InMemoryProfileRepository) SaveNew(p Profile) (err error) {
	...
	if r.findByUsername(p.Username) != nil {
		return errors.NewAlreadyExistsError("username already taken")
	}
	if r.findByEmail(p.Email) != nil {
		return errors.NewAlreadyExistsError("profile with the email already exists")
	}
...
func (r *InMemoryProfileRepository) FindByID(id uint64) (p Profile, err error) {
	...
	pPtr := r.findByID(id)
	if pPtr == nil {
		return Profile{}, errors.NewNotFoundError("profile not found")
	}
...
func (r *InMemoryProfileRepository) GetSeveralOrderByScorePaginated(page, pageSize int) (p []Profile, err error) {
	if page < 0 {
		return nil, errors.NewInvalidFormatError("invalid page")
	}
	if pageSize < 1 {
		return nil, errors.NewInvalidFormatError("invalid page size")
	}
